...
# Get the default partition size
default_partition_size = spark.conf.get("spark.sql.files.maxPartitionBytes")

# Get the default number of partitions
default_num_partitions = spark.conf.get("spark.sql.shuffle.partitions")

# Get the broadcast join table limit
broadcast_join_limit = spark.conf.get("spark.sql.autoBroadcastJoinThreshold")

print("Executor Memory:", executor_memory)
print("Default Partition Size:", default_partition_size)
print("Default Number of Partitions:", default_num_partitions)
print("Broadcast Join Table Limit:", broadcast_join_limit)

# COMMAND ----------

# Get the number of executor cores
...
conf = SparkConf()
off_heap_size = conf.get("spark.memory.offHeap.size")
print(off_heap_size)
